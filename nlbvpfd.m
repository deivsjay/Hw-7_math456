...
y(1)=4*bv(1)-8*w(1)-(h^3)*w(1)^4+4*w(2);
%y(1)=4*bv(1)-(2+h^2)*w(1)+h^2*w(1)^2+w(2);
y(n)=4*w(n-1)-8*w(n)-(h^3)*w(n)^4+4*bv(2);
%y(n)=4*w(n-1)-(2+h^2)*w(n)+h^2*w(n)^2+bv(2);
for i=2:n-1
y(i)=4*w(i-1)-8*w(i)-(h^3)*w(i)^4+4*w(i+1);
%y(i)=4*w(i-1)-(2+h^2)*w(i)+h^2*w(i)^2+w(i+1);
end
